...
// Unused. Originally was used to check if a single date in the input is a start
// or end date
/*
 * private boolean isStart(String input) { return
 * input.equalsIgnoreCase("start"); }
 * 
 * private boolean isEnd(String input) { return input.equalsIgnoreCase("end"); }
 */
 ...
 * 
 * ArrayList<String> tempUserCommand = new
 * ArrayList<String>(Arrays.asList((userCommand)));
 * 
 * for (int i = lengthOfInput - 1; i > 0; i--) { if
 * (tempUserCommand.get(i).equals("start") ||
 * tempUserCommand.get(i).equals("end")) { tempUserCommand.remove(i); break; } }
