...
public class MainWindowController implements Initializable {
	...
	private void performOperations(String userInput) throws UIOperationException {
		try {
			...
			} else if (parser.getCommand(userInput).equals("help")) {
				//showHelp();				
	...
	private void deleteTask(String userInput) throws Exception {
		...
		snackbar.getStyle().replace("-fx-background-color: rgba(76,175,80 ,1);", " -fx-background-color: RED;");
		System.out.println(snackbar.getStyle().toString());
		snackbar.fireEvent(new JFXSnackbar.SnackbarEvent(MESSAGE_DELETE_CONFIRMED, "", 1500, (b) -> {
		}));
	...
	/*
	 * private void refreshSearch() { printedPlanner.setItems(filteredEntries);
	 * }
	 */
	private void createTask(String userInput) throws Exception {
		...
		snackbar.fireEvent(new JFXSnackbar.SnackbarEvent(MESSAGE_ADD_CONFIRMED, "", 1500, (b) -> {
		}));
	...
	private void afterSearch() {
		setCellFactory(filteredEntries);
		refreshSearch();
		commandBox.clear();
	}
	...
	private void setCellFactory(ObservableList<Task> entries) {
		printedPlanner.setCellFactory(param -> {

			TaskListCell listCell = new TaskListCell(entries);
