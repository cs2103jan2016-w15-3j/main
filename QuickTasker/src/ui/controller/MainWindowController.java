...
public class MainWindowController implements Initializable {
	...
	private void handleEnterKeyPressed(KeyEvent event) {
		...
		try {
			...
			logger.log(Level.SEVERE, "Error occured at " + getClass().getName() + " within performOperation method.\n");
			e.printStackTrace();
		}
	}
	...
	private void performOperations(String userInput) throws UIOperationException {
		...
		try {
			...
			else if (parser.getCommand(userInput) == Commands.CHANGE_DIRECTORY)
	...
	/*
	 * @@author A0133333U-unused //unused because another team member had
	 * implemented search as well
	 */
	/*
	 * private void searchTask(String userInput) { if (userInput != null) {
	 * listView.setItems(plannerEntries); } String[] splitted =
	 * userInput.toLowerCase().split(" "); filteredEntries =
	 * FXCollections.observableArrayList(); listView.setItems(plannerEntries);
	 * 
	 * for (Task task : listView.getItems()) { boolean match = false; String
	 * entryText = task.getName(); System.out.println(entryText); for (int i =
	 * 1; i < splitted.length; i++) { if
	 * (entryText.toUpperCase().contains(splitted[i].toUpperCase())) { match =
	 * true; } if (match) { filteredEntries.add(task); snackbar.fireEvent(new
	 * JFXSnackbar.SnackbarEvent("Search results returned.", "", 1500, (b) -> {
	 * })); } } listView.setItems(filteredEntries); afterSearch();
	 * 
	 * } }
	 */
	...
	private void deleteTaskOperation(String userInput) {
		plannerEntries = FXCollections.observableArrayList(operations.deleteTask(parser.getTaskIndex(userInput)));
	}
	...
	private boolean isTimeSlotClashing(Task task) {
		...
		for (Task taskInList : plannerEntries)
			if (taskInList.getDueDate().equals(task.getDueDate())
				...
				} else if (endTimeWithinStartAndEnd(task, $, taskInList)
						|| endTimeWithinStartAndEnd(taskInList, $, task)
						|| startTimeWithinStartAndEnd(task, $, taskInList)
	...
	private void setCellFactory() {
		printedPlanner.setCellFactory(param -> {
			...
			printedPlanner.addEventFilter(TASK_COMPLETE, event -> new Thread(() -> {
				Thread.currentThread().setUncaughtExceptionHandler((task, e) -> Platform.runLater(System.out::println));
