...
public class MainWindowController implements Initializable {
	...
	// This method will log the user input, as well as passes valid input to the
...
//@@author A0121558H
	...
	// @@author A0133333U
	// this method will set the generic task icon in the header titles, except
	// in overdue
	...
	// @@author A0133333U
	// this method will set and assign the warning icon to the header title when
	// overdue tasks are shown
	...
	private boolean isTimeSlotClashing(Task task) {
		...
		for (Task taskInList : plannerEntries)
			if (taskInList.getDueDate().equals(task.getDueDate())
				...
				} else if (endTimeWithinStartAndEnd(task, taskInList) || endTimeWithinStartAndEnd(taskInList, task)
	...
	private boolean startTimeWithinStartAndEnd(Task task, Task taskInList) {
		...
		if (task.getStartTime() != null) {
			result = task.getStartTime().isAfter(taskInList.getStartTime().minusHours(OFFSET_FOR_CHECKING_TIME))
					&& task.getEndTime().isBefore((taskInList.getEndTime().plusHours(OFFSET_FOR_CHECKING_TIME)));
		}
		return result;
	}
	...
	private boolean endTimeWithinStartAndEnd(Task task, Task taskInList) {
		boolean result = false;
		if (task.getEndTime() != null) {
			result = task.getEndTime().isAfter(taskInList.getStartTime().minusHours(OFFSET_FOR_CHECKING_TIME))
					&& task.getEndTime().isBefore(taskInList.getEndTime().plusHours(OFFSET_FOR_CHECKING_TIME));
		}
		return result;
	}
